      ...
      tree.addTreeSelectionListener(new TreeSelectionListener() {
        public void valueChanged(TreeSelectionEvent e) {
          ...
          if (selectedNode.equals(rootNode)) {
            ...
            resource = mSystem;
          } else if (selectedNode.equals(mResourceNode)) {
            if (mResource == null)
              mResource = new MResource();

            resource = mResource;
          } else if (selectedNode.equals(mForeignKeyNode)) {
            if (mForeignKey == null)
              mForeignKey = new MForeignKey();

            resource = mForeignKey;
          } else if (selectedNode.equals(mClassNode)) {
            if (mClass == null)
              mClass = new MClass();

            resource = mClass;
          } else if (selectedNode.equals(mTableNode)) {
            if (mTable == null)
              mTable = new MTable();

            resource = mTable;
          } else if (selectedNode.equals(mUpdateNode)) {
            if (mUpdate == null)
              mUpdate = new MUpdate();

            resource = mUpdate;
          } else if (selectedNode.equals(mUpdateTypeNode)) {
            if (mUpdateType == null)
              mUpdateType = new MUpdateType();

            resource = mUpdateType;
          } else if (selectedNode.equals(mUpdateHelpNode)) {
            if (mUpdateHelp == null)
              mUpdateHelp = new MUpdateHelp();

            resource = mUpdateHelp;
          } else if (selectedNode.equals(mObjectNode)) {
            if (mObject == null)
              mObject = new MObject();

            resource = mObject;
          } else if (selectedNode.equals(mSearchHelpNode)) {
            if (mSearchHelp == null)
              mSearchHelp = new MSearchHelp();

            resource = mSearchHelp;
          } else if (selectedNode.equals(mEditMaskNode)) {
            if (mEditMask == null)
              mEditMask = new MEditMask();

            resource = mEditMask;
          } else if (selectedNode.equals(mLookupNode)) {
            if (mLookup == null)
              mLookup = new MLookup();

            resource = mLookup;
          } else if (selectedNode.equals(mLookupTypeNode)) {
            if (mLookupType == null)
              mLookupType = new MLookupType();

            resource = mLookupType;
          } else if (selectedNode.equals(mValidationLookupNode)) {
            if (mValidationLookup == null)
              mValidationLookup = new MValidationLookup();

            resource = mValidationLookup;
          } else if (selectedNode.equals(mValidationLookupTypeNode)) {
            if (mValidationLookupType == null)
              mValidationLookupType = new MValidationLookupType();

            resource = mValidationLookupType;
          } else if (selectedNode.equals(mValidationExternalNode)) {
            if (mValidationExternal == null)
              mValidationExternal = new MValidationExternal();

            resource = mValidationExternal;
          } else if (selectedNode.equals(mValidationExternalTypeNode)) {
            if (mValidationExternalType == null)
              mValidationExternalType = new MValidationExternalType();

            resource = mValidationExternalType;
          } else if (selectedNode.equals(mValidationExpressionNode)) {
          ...
          mEditButtonListener = new ActionListener() {
            public void actionPerformed(ActionEvent e) {
              ...
              if (dialog.getResponse() == JOptionPane.OK_OPTION) {
                ...
                } else if (resource instanceof MClass) {
                  mClass = new MClass();
                  refreshMainPanel(mClass);
                } else if (resource instanceof MEditMask) {
                  mEditMask = new MEditMask();
                  refreshMainPanel(mEditMask);
                } else if (resource instanceof MForeignKey) {
                  mForeignKey = new MForeignKey();
                  refreshMainPanel(mForeignKey);
                } else if (resource instanceof MLookup) {
                  mLookup = new MLookup();
                  refreshMainPanel(mLookup);
                } else if (resource instanceof MLookupType) {
                  mLookupType = new MLookupType();
                  refreshMainPanel(mLookupType);
                } else if (resource instanceof MObject) {
                  mObject = new MObject();
                  refreshMainPanel(mObject);
                } else if (resource instanceof MSearchHelp) {
                  mSearchHelp = new MSearchHelp();
                  refreshMainPanel(mSearchHelp);
                } else if (resource instanceof MSystem) {
                  mSystem = new MSystem();
                  refreshMainPanel(mSystem);
                } else if (resource instanceof MTable) {
                  mTable = new MTable();
                  refreshMainPanel(mTable);
                } else if (resource instanceof MUpdate) {
                  mUpdate = new MUpdate();
                  refreshMainPanel(mUpdate);
                } else if (resource instanceof MUpdateHelp) {
                  mUpdateHelp = new MUpdateHelp();
                  refreshMainPanel(mUpdateHelp);
                } else if (resource instanceof MUpdateType) {
                  mUpdateType = new MUpdateType();
                  refreshMainPanel(mUpdateType);
                } else if (resource instanceof MValidationExpression) {
                  mValidationExpression = new MValidationExpression();
                  refreshMainPanel(mValidationExpression);
                } else if (resource instanceof MValidationExternal) {
                  mValidationExternal = new MValidationExternal();
                  refreshMainPanel(mValidationExternal);
                } else if (resource instanceof MValidationExternalType) {
                  mValidationExternalType = new MValidationExternalType();
                  refreshMainPanel(mValidationExternalType);
                } else if (resource instanceof MValidationLookup) {
                  mValidationLookup = new MValidationLookup();
                  refreshMainPanel(mValidationLookup);
                } else if (resource instanceof MValidationLookupType) {
                  mValidationLookupType = new MValidationLookupType();
